...
# def generate_X_with_condition(d, n, lam, Q_desired):
#     mu = 1.0
#     L = Q_desired * mu
#
#     eig_min = (mu - 2 * lam) / 2
...
# lam = 0.1
# Q_desired = 100
# X_train = generate_X_with_condition(d, n, lam, Q_desired)
#
# np.savetxt("sgd/d50_mu1_L50_w_star", w_star)
# np.savetxt("sgd/d50_mu1_L50_eigvals", eigvals)
#
# # Check the Hessian:
# H = 2 * (X_train.T @ X_train) / n + 2 * lam * np.eye(d)
...
# print(f"Actual condition number: {Q_actual:.2f}")
# print("L =", L)
# print("mu =", mu)
...
X, y, w_star, H, eigvals_H = generate_X_with_condition(
    d=25, n=100000, lam=0.01, Q_desired=65, noise_std=0.1, seed=42
)

np.savetxt("d25_mu1_L50_w_star", w_star)
np.savetxt("d25_mu1_L50_eigvals", eigvals_H)

df = pd.DataFrame(X, columns=[f"x{i}" for i in range(X.shape[1])])
df["y"] = y

# Save to CSV
df.to_csv("d25_mu1_L50.csv", index=False)
